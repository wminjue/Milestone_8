...
```{r one, echo = FALSE, message= FALSE, include= FALSE}
# Similar for Gin Tama
one <- comments %>%
  select(name, episodes, rating, members) %>%
  dplyr::filter(grepl('One Piece', name))
...
masterone <- merge(one, rawanimone, by.rawanimone = "name") %>%
  arrange(start_date)


masterone$actmem <- ifelse(
  (
    (masterone$scored_by > masterone$members)
  ),
  masterone$scored_by,
  masterone$members
)

masterone$actscor <- ifelse(
  (
    (masterone$scored_by > masterone$members)
  ),
  masterone$score,
...
```{r onerun, echo= FALSE, include= FALSE}
oner <- ggplot(data = masterone, aes(x= start_date, y=actscor)) + ylim(0, 10) + geom_point(aes(x = start_date, y = actscor, color = "Score (1-10 Scale)", size = actmem)) + geom_smooth(method = "lm", aes(x = start_date, y = actscor, color = "Score (1-10 Scale)")) + labs(size = "Number of Users Providing Feedback", color = "Anime Metric", x = "Year of Anime Release", y = "Score out of 10", title = "Change in Rank of Franchise Anime Productions", subtitle = "Based on Compiled User Feedback on MyAnimeList from 2016-2017") 

write_rds(oner, "images/oner.rds")

onep <- ggplot(data = masterone, aes(x = start_date, y=actscor)) + geom_point(aes(x = start_date, y = rank, color = "Popularity", size = actmem)) + geom_smooth(method = "lm", aes(x = start_date, y = rank, color = "Popularity"))  + labs(size = "Number of Users Providing Feedback", color = "Anime Metric", x = "Year of Anime Release", y = "Popularity", title = "Change in Popularity of Franchise Anime Productions", subtitle = "Based on Compiled User Feedback on MyAnimeList from 2016-2017") 

write_rds(onep, "images/onep.rds") 
```

```{r c, echo = FALSE, message= FALSE, include= FALSE}
# Similar for Gin Tama
c <- comments %>%
  select(name, episodes, rating, members) %>%
  dplyr::filter(grepl('Detective Conan', name))
...
masterc <- merge(one, rawanimc, by.rawanimc = "name") %>%
  arrange(start_date)


masterc$actmem <- ifelse(
  (
    (masterc$scored_by > masterc$members)
  ),
  masterc$scored_by,
  masterc$members
)

masterc$actscor <- ifelse(
  (
    (masterc$scored_by > masterc$members)
  ),
  masterc$score,
...
```{r crun, echo= FALSE, include= FALSE}
cr <- ggplot(data = masterc, aes(x= start_date, y=actscor)) + ylim(0, 10) + geom_point(aes(x = start_date, y = actscor, color = "Score (1-10 Scale)", size = actmem)) + geom_smooth(method = "lm", aes(x = start_date, y = actscor, color = "Score (1-10 Scale)")) + labs(size = "Number of Users Providing Feedback", color = "Anime Metric", x = "Year of Anime Release", y = "Score out of 10", title = "Change in Rank of Franchise Anime Productions", subtitle = "Based on Compiled User Feedback on MyAnimeList from 2016-2017") 

write_rds(oner, "images/cr.rds")

cp <- ggplot(data = masterc, aes(x = start_date, y=actscor)) + geom_point(aes(x = start_date, y = rank, color = "Popularity", size = actmem)) + geom_smooth(method = "lm", aes(x = start_date, y = rank, color = "Popularity"))  + labs(size = "Number of Users Providing Feedback", color = "Anime Metric", x = "Year of Anime Release", y = "Popularity", title = "Change in Popularity of Franchise Anime Productions", subtitle = "Based on Compiled User Feedback on MyAnimeList from 2016-2017") 

write_rds(cp, "images/cp.rds") 
```

```{r binding, echo = FALSE}

#I made a dataset combining all of the popular anime franchise data to look at overall trends in popularity over the years -- maybe there is a collective correlation?

total <- rbind(masterblea, masterfma, mastergintam, masterkeroro, masternar, masterone, masterc)

#I plot this data with a linear model and see that there is actually a negative trend, suggesting spinoff productions are actually gaining traction with audiences despite the heavy weight pulling of the original franchise popularity!

totality <- ggplot(data = total, aes(x = start_date, y=actscor)) + geom_point(aes(x = start_date, y = rank, size = actmem)) + geom_smooth(method = "lm", aes(x = start_date, y = rank))  + labs(size = "Number of Users Providing Feedback", color = "Anime Metric", x = "Year of Anime Release", y = "Popularity", title = "Correlation between Year Released and Popularity of Anime Productions", subtitle = "Based on Compiled User Feedback on MyAnimeList and Anime News Network 2016-2017") 

write_rds(totality, "images/totality.rds")

# Using an lm model, I find that indeed while the intercept is around 5000, which is not terrible given there are over 70,000 anime in my combined master dataset, the coefficient is negative, suggesting as the start date grows closer to modern time, audience enthusiasm for spinoff franchises have increased, leading to higher (or a lower number) popularity rank. 
analysis <- lm(formula = rank ~ start_date, data = total)
analysis
```
